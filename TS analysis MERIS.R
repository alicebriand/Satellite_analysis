...
model_MERIS_1998_median <- lm(median_spm ~ date, data = MERIS_2002_2012)
p_value_MERIS_1998_median <- summary(model_MERIS_1998_median)$coefficients[2, 4]  # p-value pour la pente
intercept_MERIS_1998_median <- coef(model_MERIS_1998_median)[1]
slope_MERIS_1998_median <- coef(model_MERIS_1998_median)[2]

ggplot(data = MERIS_2002_2012, aes(x = date, y = median_spm)) +
  geom_smooth(method = "lm", se = TRUE, color = "darkslateblue", fill = "pink", alpha = 0.2) +
  geom_line(color = "red3") +
  annotate(
    "text",
    x = max(MERIS_2002_2012$date, na.rm = TRUE),
    y = max(MERIS_2002_2012$median_spm, na.rm = TRUE) * 0.9,
    label = paste0(
      "y = ", round(intercept_MERIS_1998_median, 3), " + ", round(slope_MERIS_1998_median, 7), " * x",
      "\n", "p = ", ifelse(p_value_MERIS_1998_median < 0.001, "< 0.001", format(p_value_MERIS_1998_median, digits = 3))
    ),
    hjust = 1,  # Alignement à droite
    vjust = 1,  # Alignement en haut
    size = 6
  ) +
  labs(title = "Evolution de la concentration en matière particulaire en suspension médiane entre 1998 et 2025 avec le produit MERIS issu de ODATIS-MR",
       x = "Date",
...
model_log_MERIS_1998_mean <- lm(log10(mean_spm) ~ date, data = MERIS_filtered)
p_value_log_MERIS_1998_mean <- summary(model_log_MERIS_1998_mean)$coefficients[2, 4]

intercept_log_MERIS_1998_mean <- coef(model_log_MERIS_1998_mean)[1]
slope_log_MERIS_1998_mean <- coef(model_log_MERIS_1998_mean)[2]
p_value_log_MERIS_1998_mean <- summary(model_log_MERIS_1998_mean)$coefficients[2, 4]

# Formater l'équation
equation_text_log <- paste0(
  "log10(y) = ", round(intercept_log_MERIS_1998_mean, 4),
  ifelse(sign(slope_log_MERIS_1998_mean) == 1, " + ", " - "),
  abs(round(slope_log_MERIS_1998_mean, 4)), " * x",
  "\n",  # Saut de ligne
  "p-value = ", format.pval(p_value_log_MERIS_1998_mean, digits = 3)
)
...
ggplot(data = MERIS_filtered, aes(x = date, y = mean_spm)) +
  ...
  geom_line(color = "red3") +
  labs(
    title = "Evolution de la concentration en matière particulaire en suspension médiane entre 1998 et 2025 (échelle log) avec MERIS",
    x = "Date",
