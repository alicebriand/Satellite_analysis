# script analyse panache sextant
...
## extraction de la résolution depuis le fichier NetCDF -------------------
...
data_crue_panache <- data_crue_panache |>
  rowwise() |>
  mutate(
    jours_depuis_crue = {
      diffs <- as.numeric(date - dates_crue)
...
kruskal.test(aire_panache_km2 ~ periode, data = data_crue_panache)

dunn.test(data_crue_panache$aire_panache_km2,
          data_crue_panache$periode,
          method = "bonferroni")
...
courbe_reponse <- data_crue_panache |>
...
ggplot(courbe_reponse, aes(x = jours_depuis_crue, y = mean_panache)) +
  # Zone avant/pendant/après
  annotate("rect",
           xmin = -5, xmax = 0,
           ymin = -Inf, ymax = Inf,
  ...
  # Labels zones
  annotate("text",
           x = -2.5, y = max(courbe_reponse$mean_panache, na.rm = TRUE) * 0.98,
           label = "Avant crue", color = "steelblue",
           fontface = "italic", size = 3.5, hjust = 0.5) +
  annotate("text",
           x = 5, y = max(courbe_reponse$mean_panache, na.rm = TRUE) * 0.98,
           label = "Après crue", color = "firebrick",
           fontface = "italic", size = 3.5, hjust = 0.5) +
  annotate("text",
           x = 0.3,
           y = max(courbe_reponse$mean_panache, na.rm = TRUE) * 0.88,
           label = "J = 0\n(début crue)",
  ...
  labs(
    ...
    subtitle = "Produit SEXTANT OC5 (1998–2025) · enveloppe = ± 1 écart-type",
...
ggplot(data_crue_panache |> filter(!is.na(periode), aire_panache_km2 > 0),
  ...
  labs(
    ...
    subtitle = "Produit SEXTANT OC5 (1998–2025) · échelle logarithmique · valeurs nulles exclues",
    x        = NULL,
  ...
  theme(
    plot.title       = element_text(face = "bold", size = 13, margin = margin(b = 4)),
    plot.subtitle    = element_text(color = "grey40", size = 10, margin = margin(b = 8)),
    axis.text.x      = element_text(size = 11, color = "grey20", face = "bold"),
